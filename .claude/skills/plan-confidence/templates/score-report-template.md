...
```json
{
  ...
  "risco_estrutural_score": 80.0,
  "active_dimensions": ["completude", "risco_estrutural"],
...
- **risco_estrutural_score** (float 0-100): Risco técnico score, M2 part only (smells).
- **active_dimensions** (list[str]): dimensions actually scored. M2: `["completude", "risco_estrutural"]`. M3+ will add `"evidencia"`. M5+ will add `"calibracao"`.
- **weight_normalization_factor** (float): ADR D8. Factor used to normalize SOTA weights (0.30/0.30/0.20/0.20) to active dimensions. In M2 with `(0.30 + 0.20) / 0.50 = 1.0` → factor for completude is `0.30/0.50 = 0.6`; for risco_estrutural is `0.20/0.50 = 0.4`.
...
Max 3 entries per dimension (top contributors). Empty list for dimensions not yet active (M2: evidencia, calibracao).
...
Completude: {completude_score}/100
  + {motivos.completude[0].label} (+{weight})
  + {motivos.completude[1].label} (+{weight})
  - {motivos.completude[2].label} ({weight})

Risco Estrutural: {risco_estrutural_score}/100
  ...
...
M2 only populates `completude` and `risco_estrutural`. The `evidencia` and `calibracao` lists are intentionally empty until M3 and M5 respectively activate them.
