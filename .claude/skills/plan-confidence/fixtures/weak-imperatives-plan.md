---
type: plan
created_at: 2026-05-17
slug: weak-imperatives-plan
status: draft
---

# Plan: Weak Imperatives Fixture (smells, no hard cap)

> Version 1.0 — Coverage OK e ADRs OK, mas com varias ocorrencias de should/could/may/might e algumas loopholes. Risco-estrutural reduzido por penalties.

## Context

Toy. O sistema should handle this. Could be improved. May fail.

## Objective

This is the objective. That is important. The implementation should work if possible.

## ADRs

### D1 — Toy

- **Decisão:** Toy.
- **Rationale:** Alternativa rejeitada: doing nothing where feasible.
- **Consequências:** As appropriate.

## Dependency Graph

```
T1.1
```

## Phase 1

### T1.1 — Implement feature

#### Objective
This may work. Should be careful.

#### Files to edit
```
a.py — could be split
```

#### TDD
```
RED: test_feature — this might fail
GREEN: implement when applicable
```

#### Acceptance Criteria
- [ ] Should pass tests

#### DoD
- [ ] May be done

## Coverage Matrix

| # | Gap / Requirement | Task(s) | Resolution |
|---|---|---|---|
| 1 | The need | T1.1 | This should resolve |

**Coverage: 1/1 (100%)**

## Global DoD

- [ ] Could be done
